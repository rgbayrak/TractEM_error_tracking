...
def open_loop(new_path,old_path,list_path,file_name,short_name,i,tract_number):
     ...
     tract_list = sheet1.col_values(0)
     folder_list=sheet1.col_values(1)
     ...
     if tractnumber==1:
         program='/nfs/masi/bayrakrg/dsistudio/dsi_other_files/dsi_studio --action=vis --source=%s/%d.fib.gz --track="%s,%s" --cmd=" " --stay_open=1'%(os.path.join(old_path,folder_list[i]),number,os.path.join(new_path1,tract_list[i]+'_tract.trk.gz'),os.path.join(old_path1,tract_list[i]+'_tract.trk.gz'))
         subprocess.call(program, shell=True)
     else:
         program = '/nfs/masi/bayrakrg/dsistudio/dsi_other_files/dsi_studio --action=vis --source=%s/%d.fib.gz --track="%s,%s" --cmd=" " --stay_open=1' % (os.path.join(old_path,folder_list[i]),
         number, os.path.join(new_path1, tract_list[i] + '_L_tract.trk.gz'),
         os.path.join(old_path1, tract_list[i] + '_L_tract.trk.gz'))
         program1= '/nfs/masi/bayrakrg/dsistudio/dsi_other_files/dsi_studio --action=vis --source=%s/%d.fib.gz --track="%s,%s" --cmd=" " --stay_open=1' % (os.path.join(old_path,folder_list[i]),
             number, os.path.join(new_path1, tract_list[i] + '_R_tract.trk.gz'),
...
list_path='/home-nfs/masi-shared-home/home/local/VANDERBILT/bayrakrg/masimatlab/trunk/users/bayrakrg/tractem/error_tracking/mat_files/BLSA18_above0_below40.xlsx'
i=0
