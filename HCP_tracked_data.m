...
% HCP_subject_path = '/home-local/bayrakrg/Dropbox (VUMC)/complete_HCP_subjects';
HCP_subject_path = '/nfs/masi/bayrakrg/tractem_data/corrected/HCP';
HCP_subject_dirs = dir(HCP_subject_path);
...
filenames = cellstr(char(HCP_subject_dirs.name));
dropfiles = false(length(HCP_subject_dirs),1);
...
for subj = 1:length(HCP_subject_dirs)
    ...
    disp(HCP_subject_dirs(subj).name);
    
    % preprocess
    tract_dirs = dir(fullfile(HCP_subject_dirs(subj).folder,HCP_subject_dirs(subj).name,'preprocess'));
    ...
    if ~isempty(tract_dirs)        
        ...
        dropfolders(~contains(tractfolders, {'_fib.gz', '_fib_qa.nii.gz','log', 'density','postproc','QA'})) = true; 
        ...
        for trk = 1:length(tract_dirs)
            ...
            if length(files) > 2
                ...
                tractem_data = struct;
                tractem_data.subject_rater = HCP_subject_dirs(subj).name;
...
%                 tractem_data.extra = error_files;
    ...
    % postprocess
    tract_dirs = dir(fullfile(HCP_subject_dirs(subj).folder,HCP_subject_dirs(subj).name,'*'));
    tract_dirs = tract_dirs(~ismember({tract_dirs.name},{'.','..'}));
     ...
     if ~isempty(tract_dirs)        
        ...
        tractfolders = cellstr(char(tract_dirs.name));
        dropfolders = false(length(tract_dirs),1);
        dropfolders(~contains(tractfolders, {'.', '..', '_fib.gz', '_fib_qa.nii.gz','density','postproc', 'preprocess','QA'})) = true; 
        tract_dirs = tract_dirs(dropfolders);
     ...
     if ~isempty(tract_dirs)        
        ...
        for trk = 1:length(tract_dirs)
            ...
            if ~isempty(files)
                init = cellstr(char(files.name));
                drop1 = false(length(files), 1);            
                drop1(~contains(init, {'log', 'corrected_ROAs'})) = true;
                files = files(drop1); 
            end
            ...
            if length(files) > 3
                ...
                % deal with default and weird files
                filelist = cellstr(char(files.name));
                drop2 = false(length(files), 1);
                drop2(contains(filelist, abb, 'IgnoreCase', true)) = true;
                files = files(drop2);              
...
sum([orig.hcp_data.error_count])
orig.hcp_data([orig.hcp_data.error_count]>0)

corr = load('hcp_corrected_data.mat');
sum([corr.hcp_data.error_count])
corr.hcp_data([corr.hcp_data.error_count]>0)
